...
public class Hough implements PlugInFilter {
	...
	double pTemp;
	double distance = 2*(Math.sqrt(width*width + height*height)*Math.sqrt(2));
	int[][] houghArray = new int[180][(int)distance];
	for (int y=0; y<height; y++)
	{
		for (int x=0; x<width; x++)
		{
			if (canny.getPixelValue(x, y)>0)
			{
				for(int theta=-90; theta<90; theta++)
				{
					pTemp = x*Math.cos(Math.toRadians(theta)) + y*Math.sin(Math.toRadians(theta));
					houghArray[theta+90][(int) (pTemp+(distance/2))]++;
				} 
			}
		}
	}
	...
	ImageProcessor lines = new FloatProcessor(width, height);

	for (int y=0; y<height; y++)
	{
		for (int x=0; x<width; x++)
		{
			lines.putPixelValue(x, y, 0);	
		}
	}
	...
	for(int theta=-90; theta<90; theta++)
	{
		for (int p=0; p<(int) distance; p++)
		{
			if (houghArray[theta+90][p]>=75)
			{
				for(int x=0; x<width; x++)
				{
					temp = p-(distance/2) - x*Math.cos(Math.toRadians(theta)); 
					y = (int)(temp/(Math.sin(Math.toRadians(theta))));
					if (canny.getPixelValue(x, y)==0)
					{
						connected = false;
						for(int i=x+1; i<x+5; i++)
						{	
							temp = p-(distance/2) - i*Math.cos(Math.toRadians(theta)); 
							y = (int)(temp/(Math.sin(Math.toRadians(theta))));
							if (canny.getPixelValue(i, y)>0)
							{
								connected=true;
								connectTemp=i;
							}
						...
						if (connected)
						{
							for(int i=x+1; i<=connectTemp; i++)
							{	
								temp = p-(distance/2) - i*Math.cos(Math.toRadians(theta)); 
								y = (int)(temp/(Math.sin(Math.toRadians(theta))));
								lines.putPixelValue(i, y, 255);
								canny.putPixelValue(i, y, 255);
							}
						}
	...
	ImagePlus imp2 = new ImagePlus("Hough", lines);
	imp2.show();
