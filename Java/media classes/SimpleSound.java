...
public class SimpleSound{
  ...
  public void increaseVolume()
  {
    ...
    sampleArray = getSamples();
    
    for(int index=0; index < sampleArray.length; 
            index++)
    {
  ...
  public void changeVolume(double amount)
  {
            ...
            index++)
    {
       sample = sampleArray[index];
       value = sample.getValue();
       sample.setValue((int) (value*amount));
    }
  ...
  {
    Sound copy = new Sound(getFileName());
    int samplesLength = getNumSamples();
   
    for(int i = samplesLength-1; i > 0; i--)
    {    
      setSampleValueAt(samplesLength-i,
                       copy.getSampleValueAt(i));
    }
  ...
  public void doubleFrequency()
  {
    ...
    {
      setSampleValueAt(i/2, copy.getSampleValueAt(i));
    }
  
    // students should see a problem with the above in that
    // it does not clear out the remaining part of the leftover
    // part of the file -> the end is still in original form
    // The fix is to clear out the second half of the array to 0
    
  }

  // only does half of the original 
  public void halfFrequency()
  {
    Sound s = new Sound(getFileName());
    
    for(double source=0, target=0; source < getLength() / 2;
        source=source + 0.5, target++)
    {
      setSampleValueAt((int) target, s.getSampleValueAt((int) source));
    }  
  ...
  public void echo(int delay)
  {
    ...
    for(int i=delay; i < getLength(); i++)
    {
      // echo value is fainter at delay location
  ...
  {
    Sound target = new Sound(getFileName());
