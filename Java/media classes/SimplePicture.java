...
// start of my custom methods///////////////////***////////////////////***///////////////////////***/////////////////////////
 ...
 public void decreaseBlue(double percent)
 {
   ...
   for(Pixel pixObj : pixArray)
   {
     value = pixObj.getBlue();
     value = (int) (value * percent);
     pixObj.setBlue(value);
   }
 }
 public void decreaseGreen(double percent)
 {
   ...
   for(Pixel pixObj : pixArray)
   {
     value = pixObj.getGreen();
     value = (int) (value * percent);
     pixObj.setGreen(value);
   }
 }
...
{ 
 {
  ...
  for(Pixel pixObj : pixArray)
   {
     valueGreen = pixObj.getGreen();
     valueRed = pixObj.getRed();
     valueBlue = pixObj.getBlue();
     totalValue = ((valueGreen + valueRed + valueBlue) / 3);
  
     pixObj.setGreen(totalValue);
     pixObj.setRed(totalValue);
     pixObj.setBlue(totalValue);
   }
 }
}

public void brightenPhoto()
{
  Pixel pixel;
  Color aColor;
  ...
  for(Pixel pixObj : pixArray)
   {
     ...
     valueRed = pixObj.getRed();
     valueBlue = pixObj.getBlue();
     totalValue = (valueGreen + valueRed + valueBlue);
...
public void copyTo(Picture target)
{
  Pixel srcPix;
  Pixel trgPix;
  Color aColor;
  ...
  { 
    ...
    {
      srcPix = getPixel(x,y);
      trgPix = target.getPixel(x,y);
      aColor = srcPix.getColor();
      trgPix.setColor(aColor);
    }
  }
}

public void copyToGreen(Picture target)
{
  Pixel srcPix;
  Pixel trgPix;
  Color aColor;
  ...
  { 
    ...
    for (int y = 0; y < getHeight(); y++)
    {
      ...
      srcPix = getPixel(x,y);
      if (srcPix.getRed() + srcPix.getBlue() > (srcPix.getGreen() - 20))
      {
        trgPix = target.getPixel(x,y);
        aColor = srcPix.getColor();
        trgPix.setColor(aColor);
...
public void flipV(int a)
{
    ...
    for(int x = 0; x < getWidth()/2; x++) 
  {
   ...
   Pixel aNewPixel = getPixel(dist, y);
     
      if(a == 1) 
     
      {
         aNewPixel.setColor(pixel.getColor());
      }
     
         if(a ==2)
     
     {
          pixel.setColor(aNewPixel.getColor());
     }
...
public void scaleDown(Picture target)
{

  Pixel srcPix, trgPix;
  Color aColor;
  
  for(int srcx=0, trgx=0; srcx < getWidth();
      srcx = srcx + 2, trgx++)
  {
        ...
        srcy = srcy + 2, trgy++)
    {
      ...
      srcPix = getPixel(srcx,srcy);
      trgPix = target.getPixel(trgx,trgy);
      aColor = srcPix.getColor();
      trgPix.setColor(aColor);
...
public void scaleUp(Picture target)
{
  ...
  for(int srcx=0, trgx=0, trgx2=1; srcx < getWidth();
      srcx = srcx + 1, trgx = trgx + 2, trgx2 = trgx2 + 2)
  {
  
    for(int srcy=0, trgy=0, trgy2=1; srcy < getHeight();
        srcy = srcy + 1, trgy = trgx + 2, trgy2 = trgy2 + 2)
    {
    
      srcPix = getPixel(srcx,srcy);
      trgPix = target.getPixel(trgx,trgy);
      trgPix = target.getPixel(trgx2,trgy2);
      aColor = srcPix.getColor();
      trgPix.setColor(aColor);
...
public void redEyeRemoval(int uL1, int lR1, int uL2, int lR2)
{ 
  ...
  for(int leftX = getWidth(); uL1 < leftX && leftX < lR1;)
  {
  
    for(int leftY = getHeight(); uL1 < leftY && leftY < lR1;)
    {
    
     leftEye = getPixel(leftX,leftY);
     leftEyeColor = leftEye.getColor();
     
     if (leftEyeColor.getRed() > leftEyeColor.getBlue() + leftEyeColor.getGreen())
      {
  ...
  for(int rightX = getWidth(); uL2 < rightX && rightX < lR2;)
  {
  
    for(int rightY = getHeight(); uL2 < rightY && rightY < lR2;)
    {
    
     rightEye = getPixel(rightX,rightY);
     rightEyeColor = rightEye.getColor();
     
     if (rightEyeColor.getRed() > rightEyeColor.getBlue() + rightEyeColor.getGreen())
      {
...
public void edgeDetection(double threshhold)
{
  ...
  for(int x=0; x< getWidth(); x= x +1)
  {
    ...
    for (int y=0; y< getHeight() - 1; y++)
    {
      ...
      topPxl = getPixel(x,y);
      bottomPxl = getPixel(x, y+1);
      
      topAvg = topPxl.getAverage();
      bottomAvg = bottomPxl.getAverage();
      
      if(Math.abs(topAvg-bottomAvg)<threshhold)
      {  
        topPxl.setColor(Color.WHITE);
      }
      else
      {
        topPxl.setColor(Color.BLACK);
      }
    }
