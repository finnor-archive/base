public class LifeGame
{
   ...
   {
        board0 = new int[m][n];                //fixed the board sizes to match
        board1 = new int[m][n];
        M = m;
        N = n;
        generation_count = 0;
                                        ...
                                        int[][]outboard)
   {
      ...
      for(int i = 1; i < Mminus1; i++)
      {
         for(int j = 1; j < Nminus1; j++)
         {
            ...
            int counter = inboard[i][j-1]   /* left */ + inboard[i][j+1]   /* right  */ +
                          ...
                          inboard[i-1][j-1] /* UL   */ + inboard[i-1][j+1] /* UR     */ +   //UL and UR changed
      ...
      {
           // add up how many neighbors I have at cell (i,j):
            int counter = inboard[i][Nminus1-1]   + 0 /* no right */ +             //fixed all nodes to be Nminus1 versus Mminus1
                          inboard[i-1][Nminus1]   + inboard[i+1][Nminus1]  +
                          inboard[i-1][Nminus1-1] + 0 /* no UR */ +           
                          inboard[i+1][Nminus1-1] + 0 /* no LR */;            
            if(((counter == 2)&&(inboard[i][Nminus1]==1)) ||
               ((counter == 3)))
             {
               outboard[i][Nminus1] = 1;
            }
            else
            {
               outboard[i][Nminus1] = 0;
            }  
       }
