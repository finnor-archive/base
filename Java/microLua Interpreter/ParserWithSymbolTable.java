...
// the MicroLua programming language.  The parser also constructs a syntax tree
// representation of all executable constructions and an environment for
// each function.
...
public class ParserWithSymbolTable {
  ...
  public void chunk () throws java.io.IOException {
    ...
    getToken ();
    localVarDecl (envglobal);      // localvardecl
    while (token . symbol () == TokenClass . FUNCTION) { // { function
      ...
      if (token . symbol () == TokenClass . ID) {  // [ identifier
        argId = token . lexeme ();
        parameter . add (argId);
        envlocal . update (argId, new ExpressibleValue ());
        getToken ();
      ...
      if (token . symbol () != TokenClass . RETURN)   // [ 
        blockTree = block (envlocal);    // block ]
      ...
      envglobal . 
        update (functionId,
          new FunctionDenot (parameter, new Type (Type . VOID), envlocal, 
            blockTree));
      getToken ();
    }        // }
    ...
    envglobal . 
      update ("_main",
        new FunctionDenot (null, new Type (Type . VOID), envlocal, blockTree));
    System . out . println ();
    envglobal . print ("Program");
    
    
    Interpreter interpret = new Interpreter(envglobal, blockTree);
    OutputFile output = interpret.inter();
    output.print();
  }

  // localVarDecl parses local variable declarations.

  public void localVarDecl (Environment env) throws java.io.IOException {
    String varId;
    while (token . symbol () == TokenClass . LOCAL) {  // { local
      ...
      if (token . symbol () != TokenClass . ID)   // identifier
        ErrorMessage . print ("identifier EXPECTED");
      varId = token . lexeme ();
      env . update (varId, new ExpressibleValue ());
      getToken ();
      while (token . symbol () == TokenClass . COMMA) { // { ,
        getToken ();
        if (token . symbol () != TokenClass . ID)   // identifier
  ...
  public SyntaxTree block (Environment env) throws java.io.IOException {
    SyntaxTree syntaxTree;
    localVarDecl (env);     // localvardecl
